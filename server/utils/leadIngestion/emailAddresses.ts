...
export function parseInboundLeadEmail(body: Record<string, any>) {
  const text = String(body.text || body.plain || body.body || body.html || '');
  const subject = String(body.subject || 'Inbound lead');
  const fromEmail = extractEmailAddresses(body.from || body.sender || body.envelope?.from)[0] || null;
