<template>
  <div v-if="hasData" class="relative w-full min-h-[600px]">
    <!-- Loading State -->
    ...
    <div v-if="!loading && !error" class="absolute inset-0 flex items-center justify-end z-20 pointer-events-none">
      <div class="w-full lg:w-[380px] xl:w-[420px] bg-white shadow-2xl m-3 lg:m-4 rounded-lg overflow-hidden pointer-events-auto">
      <div class="p-4 lg:p-5 flex flex-col">
        ...
        <div class="mb-4" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
          <div class="flex items-center justify-between gap-3">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              class="flex flex-col items-center gap-0.5 px-3 py-2 text-[#1a73e8] font-medium text-sm leading-tight rounded-md border border-transparent hover:border-[#1a73e8]/30 hover:bg-blue-50 transition-colors flex-shrink-0"
            >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            class="w-full flex items-center justify-center gap-2 px-3 py-2 bg-gray-900 text-white hover:bg-gray-800 rounded-lg text-sm font-medium transition-colors shadow-sm"
          >
        ...
        <div class="mb-4 space-y-2" itemscope itemtype="http://schema.org/LocalBusiness">
          ...
          <a
            ...
            class="flex items-center gap-2 text-[15px] font-semibold text-gray-900 hover:text-gray-700 transition-colors leading-snug"
          >
          ...
          <div class="relative min-h-[150px]">
            ...
            >
              <div :key="activeDepartment" class="space-y-1">
                <div
                  v-for="(hours, day) in currentDepartmentHours"
                  :key="`${departments[activeDepartment]?.key || 'sales'}-${day}`"
                  class="flex justify-between items-center text-xs py-1.5 border-b border-gray-100 last:border-0"
                  :class="{ 'text-gray-900 font-medium': isToday(day), 'text-gray-800': !isToday(day) }"
                >
                  <span>{{ day }}</span>
