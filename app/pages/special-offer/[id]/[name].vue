      ...
      <ClientOnly>
        <Teleport to="body">
          <Transition name="modal">
            <div v-if="showEnquiryModal" class="enquiry-modal-overlay" @click.self="showEnquiryModal = false">
              <div class="enquiry-modal">
                <button class="enquiry-modal-close" @click="showEnquiryModal = false">
                ...
                <h2 class="enquiry-modal-title">Contact the dealer to inquire about this offer</h2>
                <p class="enquiry-modal-subtitle">{{ offer.model }} {{ offer.variantName }}</p>
...
const transformOfferData = (offersData: any, variantId: string) => {
  ...
  // Always search in category models to get the model's variants list
  if (offersData?.categories) {
    for (const category of offersData.categories) {
      for (const model of category.models) {
        const variant = model.variants?.find((v: any) => v.id === variantId);
        if (variant) {
          // If we haven't found the variant yet, use this one
          if (!foundVariant) {
            foundVariant = {
              ...variant,
          ...
          // Always capture the model's variants, image, and ID
  ...
  return {
    ...
    image: foundVariant.image || modelImage,
    heroImage: foundVariant.image || modelImage,
    imageAltText: foundVariant.imageAltText,
    modelImage: modelImage || foundVariant.modelImage,
...
.offer-cta-buttons {
  ...
  border-radius: 6px;
  overflow: hidden;
  box-shadow: 0 2px 8px rgba(0, 44, 95, 0.15);
}
...
.modal-enter-active .enquiry-modal,
.modal-leave-active .enquiry-modal {
...
.modal-enter-from .enquiry-modal,
.modal-leave-to .enquiry-modal {
