<template>
  <div class="hyundai-offers">
    ...
    <div class="offers-intro">
      <h1 class="intro-heading">Jump into offers across a wide range of new and demonstrator models.</h1>
      <p class="intro-body">Whether you're looking for a small car for the city or a bold SUV for the open road, now is the perfect time to drive away in a brand new Hyundai. Explore our current car deals and offers to start your Hyundai journey.</p>
    </div>
    ...
    <div v-else class="offers-content">
        ...
        <div class="filter-strip-inner">
          ...
          <div class="category-tabs">
            ...
            <NuxtLink
              ...
              :key="category.name"
              :to="`/special-offers/${slugify(category.name)}`"
      ...
      <div class="uk-container uk-container-large uk-margin-medium-top uk-margin-large-bottom">
        <div class="models-grid" id="models-section" ref="modelsSectionRef">
          <div 
...
interface OffersData {
  ...
  categories: Category[];
  variants: any[];
  variantsById: Record<string, any>;
  globalDisclaimers: any[];
  heading?: string;
  body?: string;
}
...
// SEO Meta - Optimized for Google Search (uses dynamic site name)
useSiteMeta({
  title: 'New Hyundai Special Offers & Deals',
  description: () => `Discover the latest Hyundai special offers, driveaway deals and promotions at ${siteName.value}. Save on new Hyundai SUVs, sedans, and electric vehicles. Limited time offers available now.`,
...
const categories = computed<Category[]>(() => {
  return offersData.value?.categories || [];
});
...
const currentModels = computed<Model[]>(() => {
  ...
  return Array.from(modelMap.values()).sort((a, b) => (a.order || 0) - (b.order || 0));
});
...
// Add OfferCatalog Schema.org markup for better search visibility
// Uses dynamic site config for vendor-agnostic implementation
const offersSchemaScript = computed(() => {
  if (!offersData.value?.categories) return null;
  
  // Get site details from store
  const dealerName = siteName.value;
  const dealerUrl = siteUrl.value;
  const dealerAddress = mainStore.site?.address || '';
  const dealerCity = mainStore.site?.city || '';
  const dealerState = mainStore.site?.state || '';
  
  // Collect all offers from all models
      ...
      for (const variant of (model.variants || [])) {
        if (variant.hasValueOffer) {
          offerItems.push({
            ...
            url: `${dealerUrl}/special-offer/${variant.id}/${slugify(model.name)}-${slugify(variant.name)}`,
            itemOffered: {
              '@type': 'Car',
              name: `${model.name} ${variant.name}`,
              brand: {
  ...
  return {
    ...
    '@type': 'OfferCatalog',
    name: `Hyundai Special Offers at ${dealerName}`,
    description: `Current special offers and deals on new Hyundai vehicles at ${dealerName}`,
    numberOfItems: offerItems.length,
    itemListElement: offerItems.slice(0, 20), // Limit to first 20 for schema size
    provider: {
      '@type': 'AutoDealer',
      name: dealerName,
      url: dealerUrl,
      address: {
        '@type': 'PostalAddress',
        streetAddress: dealerAddress,
        addressLocality: dealerCity,
        addressRegion: dealerState,
        addressCountry: 'AU',
      },
    },
  };
...
useHead({
  script: computed(() => offersSchemaScript.value ? [
    {
      type: 'application/ld+json',
      children: JSON.stringify(offersSchemaScript.value),
    },
  ] : []),
});
...
onMounted(() => {
  ...
  const stopWatcher = router.afterEach((to, from) => {
    ...
    if (to.path === '/special-offers' && shouldScrollToModels.value) {
